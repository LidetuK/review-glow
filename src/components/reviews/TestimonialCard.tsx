...
const TestimonialCard = ({ review }: TestimonialCardProps) => {
  ...
  return (
    <motion.div
      className="bg-gray-900/60 p-6 rounded-xl h-full flex flex-col min-h-[300px] border border-gray-800"
    ...
    >
      ...
      <div className="flex mb-4">
        {[...Array(5)].map((_, i) => (
          <Star 
            key={i} 
            className={`h-6 w-6 ${i < review.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-600'}`} 
          />
        ))}
      </div>
      ...
      <div className="flex items-center mt-auto">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">{jobInfo.title} • {jobInfo.company}</p>
        </div>
      </div>
    </motion.div>
  );
